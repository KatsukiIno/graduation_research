# action 時系列動作分類　リアルタイム推論 pose+hand
...
input_video_path = "input/videos/referee_action_videos/ball_in_left.mp4"
...
interpreter_pose = tf.lite.Interpreter(model_path="model/action_classifier/action_classifier.tflite")
            ...
            if results.pose_landmarks is not None:
                ...
                if class_label is not None:
                    if prov_class_label != class_label:
                        if prov_class_label == 12 and len(pose_history) > 2:  # 直前のクラスラベルが11なら
                        ...
                        if len(pose_history) > 2 and class_label == 12:
